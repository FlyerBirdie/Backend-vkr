...
def build_schedule(
    ...
    **Плановый период:** в запросе задаются **абсолютные** моменты времени в ISO-8601 с
    ...
    значения отклоняются проверкой. Операции размещаются только в пересечении периода,
    окна заказа `[planned_start, planned_end]` и **общего календаря участка** (пн–пт,
    08:00–17:00 по Europe/Samara, перерыв 12:00–13:00; оборудование в MVP — тот же календарь).
    Период по умолчанию: с полуночи текущих календарных суток в Europe/Samara на 14 суток вперёд
    (границы в ответе — в UTC).
